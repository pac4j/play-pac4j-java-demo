...
# Script to launch play-pac4j-java-demo and verify it works
...
if [ "$HTTP_CODE" = "200" ]; then
    ...
    if [ "$CAS_HTTP_CODE" = "200" ] && echo "$CAS_CONTENT" | grep -q "Enter Username & Password"; then
        ...
        EXECUTION=$(grep -o 'name="execution" value="[^"]*"' "$CAS_LOGIN_PAGE" | sed -E 's/.*value="([^"]+)".*/\1/' | head -n1 || true)
        ...
        if [ -z "$EXECUTION" ]; then
            ...
            if [ "$FINAL_APP_CODE" = "200" ]; then
                ...
                if grep -q "leleuj@gmail.com" "$FINAL_APP_PAGE"; then
                    echo "✅ Email 'leleuj@gmail.com' found in final page - Authentication successful!"
                fi
            else
...
if [ "$HTTP_CODE" = "200" ] && [ "$CAS_TEST_PASSED" = "true" ] && [ "$CAS_AUTH_PASSED" = "true" ]; then
    echo "🎉 play-pac4j-java-demo test completed successfully!"
    echo "✅ All tests passed:"
    echo "   - Application responds with HTTP 200"
    echo "   - CAS link redirects correctly to login page"
    echo "   - CAS login succeeds and demo is reachable after authentication"
else
    echo "❌ play-pac4j-java-demo test failed!"
    if [ "$HTTP_CODE" != "200" ]; then
